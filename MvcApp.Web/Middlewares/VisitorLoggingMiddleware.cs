...
namespace MvcApp.Web.Middlewares
{
    public class VisitorLoggingMiddleware
    {
        ...
        public VisitorLoggingMiddleware(RequestDelegate next, IMemoryCache cache, IConfiguration configuration, ILogger<VisitorLoggingMiddleware> logger)
        {
            ...
            _cache = cache;
            _logger = logger;
            _ipGeolocationApiKey = configuration["IpGeolocation:ApiKey"] ?? string.Empty;
            if (string.IsNullOrEmpty(_ipGeolocationApiKey))
            {
                _logger.LogWarning("IpGeolocation:ApiKey is not configured; visitor geolocation will use the default service without a key.");
            }
        }
        ...
        {
            // Skip external lookups for loopback/private addresses (e.g. local development),
            // and cap the call with a short timeout so visitor logging can never stall a request.
