...
namespace MvcApp.Identity.Pages.Account
{
    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            // Check DB settings
            var registrationEnabled = await _settings.GetAsync<bool>("EnableRegistration") ?? true;
            if (!registrationEnabled)
            {
                ModelState.AddModelError(string.Empty, "Registration is currently disabled by the administrator.");
                return Page();
            }

            var maxUsers = await _settings.GetAsync<int>("MaxUserRegistration");
            if (maxUsers.HasValue)
            {
                var currentCount = _userManager.Users.Count();
                if (currentCount >= maxUsers.Value)
                {
                    ModelState.AddModelError(string.Empty, $"Registration limit reached (max {maxUsers.Value} users).");
                    return Page();
                }
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await _emailSender.SendEmailAsync(Input.Email, "Confirm your email", "Confirm your email",
                        $"<p>Thanks for joining! Please confirm your email address to activate your account.</p>",
